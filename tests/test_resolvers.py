...
class TestObjectBuilder(object):
        ...
        User = g.ObjectType(
            ...
            fields=(
                g.field(
                    "name",
                    type=g.String,
                    params=(g.param("truncate", type=g.Int, default=None),),
                ),
            ),
        )
    ...
    def test_attr_uses_attr_name_to_resolve_field(self):
        ...
        result = object_builder(
            types.SimpleNamespace(name="Bob", email="bob@example.com")
        )
        assert_that(
            result,
            has_attrs(
    ...
    def test_when_type_is_interface_then_typename_field_is_unresolved(self):
        User = g.InterfaceType("User", fields=(g.field("name", type=g.String),))
...
class TestRootResolver(object):
    ...
    def test_root_object_resolver_passes_arguments_to_field_resolvers(self):
        Root = g.ObjectType(
            "Root",
            fields=(
                g.field("value", type=g.Int, params=(g.param("answer", type=g.Int),)),
            ),
        )
