...
class ScalarQuery(object):
    ...
    def __add__(self, other):
        if not isinstance(other, ScalarQuery):
            ...
            raise TypeError(
                "cannot add queries for different scalar types: {} and {}".format(
                    self.type,
                    other.type,
                )
            )
...
class EnumQuery(object):
    ...
    def __add__(self, other):
        if not isinstance(other, EnumQuery):
            ...
            raise TypeError(
                "cannot add queries for different enum types: {} and {}".format(
                    self.type,
                    other.type,
                )
            )
...
class InputObjectType(object):
    ...
    def _create_instance_type(self):
        ...
        def __repr__(self):
            return "{}({})".format(
                name,
                ", ".join(
                    "{}={!r}".format(key, value) for key, value in self._values.items()
                ),
            )
    ...
    def __call__(self, **explicit_field_values):
        def get_field_value(field):
            ...
            if value is _undefined:
                raise GraphError(
                    "{} is missing required field {}".format(self.name, field.name)
                )
            else:
                return value

        field_values = iterables.to_dict(
            (field.name, get_field_value(field)) for field in self.fields
        )
    ...
    def child_types(self):
        return tuple(field.type for field in self.fields)
...
class InterfaceType(object):
    ...
    def query(self, *, field_queries, create_object):
        return ObjectQuery.create(
            self, field_queries=field_queries, create_object=create_object
        )
...
class ListType(object):
    ...
    def coerce(self, value):
        return [self.element_type.coerce(element) for element in value]
...
class ListQuery(object):
    ...
    def __add__(self, other):
        if not isinstance(other, ListQuery):
            ...
            raise TypeError(
                "cannot add queries for lists with different element types: {} and {}".format(
            ...
            return ListQuery(
                type=self.type, element_query=self.element_query + other.element_query
            )

    def __str__(self):
        return _format_call_tree(
            "ListQuery",
            (
                ("type", self.type),
...
class NullableQuery(object):
    ...
    def __add__(self, other):
        if not isinstance(other, NullableQuery):
            ...
            raise TypeError(
                "cannot add queries for nullables with different element types: {} and {}".format(
                    self.type.element_type,
                    other.type.element_type,
                )
            )
...
class ObjectType(object):
    def __init__(self, name, fields, interfaces=None):
        ...
        def owned_fields():
            return tuple(field.with_owner_type(self) for field in fields())
...
def _fields_child_types(fields):
    return tuple(child_type for field in fields for child_type in field.child_types())
...
class Fields(object):
    ...
    def _find_field(self, field_name):
        return iterables.find(
            lambda field: field.name == field_name, self._fields(), default=None
        )
...
class ObjectQuery(object):
    ...
    def create(type, *, field_queries, create_object=None):
        ...
        return ObjectQuery(
            type, field_queries=field_queries, create_object=create_object
        )
    ...
    def _is_in_type_hierarchy(self, target_type):
        return (
            isinstance(self.type, InterfaceType)
            and isinstance(target_type, ObjectType)
            and self.type in target_type.interfaces
        ) or (
            isinstance(self.type, ObjectType)
            and isinstance(target_type, InterfaceType)
            and target_type in self.type.interfaces
        )

    def __str__(self):
        field_queries = _format_tuple(
            str(field_query) for field_query in self.field_queries
        )
        return _format_call_tree(
            "ObjectQuery",
            (
...
def _field_queries_for_type(field_queries, target_type):
    if isinstance(target_type, InterfaceType):
        ...
        def field_query_for_type(field_query):
            ...
            if (
                field_query.field in target_type.fields
                or field_query.field == typename_field
            ):
                return field_query
...
class Field(object):
    ...
    def with_owner_type(self, owner_type):
        return Field(
            owner_type=owner_type, name=self.name, type=self.type, params=self.params
        )
    ...
    def query(self, args, key, type_query):
        explicit_args = iterables.to_dict(
            (arg.parameter.name, arg.value) for arg in args
        )
        ...
        def get_arg(param):
            ...
            if value is _undefined:
                raise GraphError(
                    "field {} is missing required argument {}".format(
        ...
        field_args = Object(
            iterables.to_dict((param.name, get_arg(param)) for param in self.params)
        )

        return FieldQuery(
            key=key,
            field=self,
            type_query=type_query.for_type(self.type),
            args=field_args,
        )
    ...
    def child_types(self):
        return (self.type,) + tuple(param.type for param in self.params)
...
def _partition_by_type(values, types):
    ...
    for value in values:
        potential_results = [
            result for type, result in zip(types, results) if isinstance(value, type)
        ]
...
class Params(object):
    ...
    def _find_param(self, param_name):
        return iterables.find(
            lambda param: param.name == param_name, self._params, default=None
        )
...
def _format_call_tree(receiver, args):
    return "{}({}\n)".format(
        receiver, "".join(_indent("\n{}={},".format(key, value)) for key, value in args)
    )
...
def _query_coercion_error(source_type, target_type):
    return TypeError(
        "cannot coerce query for {} to query for {}".format(
            source_type,
            target_type,
        )
    )
