...
users = [UserRecord(id=index, name="user{}".format(index)) for index in range(0, int(sys.argv[1]))]
...
def graphlayer_performance():
    ...
    return lambda document_text: graphql.execute(graph=graph, document_text=document_text, query_type=Query)


def graphql_performance():
    import graphql

    User = graphql.GraphQLObjectType("User", fields={
