...
def test_params_are_accessible_with_trailing_underscore():
    params = schema.Params("book", (schema.param("class", type=schema.String),))
    assert_that(params.class_, equal_to(getattr(params, "class")))


def test_fields_are_accessible_with_trailing_underscore():
    fields = schema.Fields("Book", (schema.field("class", type=schema.String),))
...
def test_when_field_does_not_exist_on_object_type_then_error_is_raised():
    book = schema.ObjectType("Book", fields=(schema.field("title", schema.String),))
...
def test_given_input_field_has_no_default_when_input_field_is_not_set_then_error_is_raised():
    Input = schema.InputObjectType(
        "Input",
        fields=(schema.input_field("field0", type=schema.Int),),
    )
...
def test_given_field_arg_has_no_default_when_field_arg_is_not_set_then_error_is_raised():
    ...
    assert_that(
        str(error.value), equal_to("field one is missing required argument arg0")
    )
...
def test_arguments_are_coerced():
    ...
    error = pytest.raises(
        GraphError, lambda: Root.fields.one(Root.fields.one.params.arg(None))
    )
...
class TestCoerce(object):
    ...
    def test_cannot_coerce_none_to_boolean(self):
        self._assert_coercion_failure(
            schema.Boolean, None, "cannot coerce None to Boolean"
        )

    def test_cannot_coerce_true_string_to_boolean(self):
        self._assert_coercion_failure(
            schema.Boolean, "True", "cannot coerce 'True' to Boolean"
        )
        self._assert_coercion_failure(
            schema.Boolean, "true", "cannot coerce 'true' to Boolean"
        )
    ...
    def test_cannot_coerce_large_int_to_float(self):
        self._assert_coercion_failure(
            ...
            "cannot coerce 11111111111111111111111111111111111 to Float",
        )
    ...
    def test_cannot_coerce_numeric_string_to_float(self):
        self._assert_coercion_failure(
            schema.Float, "1.0", "cannot coerce '1.0' to Float"
        )
    ...
    def test_string_type_coerces_strings_to_strings(self):
        ...
        self._assert_coercion(
            schema.String, "The Ballad of Mr Steak", "The Ballad of Mr Steak"
        )

    def test_cannot_coerce_none_to_string(self):
        self._assert_coercion_failure(
            schema.String, None, "cannot coerce None to String"
        )
    ...
    def test_cannot_coerce_none_to_enum(self):
        ...
        self._assert_coercion_failure(
            DirectionGraphType, None, "cannot coerce None to Direction"
        )
    ...
    def test_cannot_coerce_string_to_enum(self):
        ...
        self._assert_coercion_failure(
            DirectionGraphType, "up", "cannot coerce 'up' to Direction"
        )
    ...
    def test_input_object_type_coerces_instances_to_same_value(self):
        UserInput = schema.InputObjectType(
            "UserInput",
            fields=lambda: (schema.input_field("name", type=schema.String),),
        )

        self._assert_coercion(UserInput, UserInput(name="Bob"), UserInput(name="Bob"))

    def test_cannot_coerce_instances_of_input_object_type_to_other_input_object_type(
        self,
    ):
        PersonInput = schema.InputObjectType(
            "PersonInput",
            fields=lambda: (schema.input_field("name", type=schema.String),),
        )
        ...
        self._assert_coercion_failure(
            UserInput,
            PersonInput(name="Bob"),
            "cannot coerce PersonInput(name='Bob') to UserInput",
        )
    ...
    def test_cannot_coerce_none_to_input_object_type(self):
        ...
        self._assert_coercion_failure(
            UserInput, None, "cannot coerce None to UserInput"
        )
...
class TestAdd(object):
    ...
    def test_adding_scalar_query_to_other_query_raises_type_error(self):
        pytest.raises(
            TypeError, lambda: schema.Boolean() + schema.ObjectType("Obj", fields=())
        )
    ...
    def test_adding_scalar_queries_of_different_type_raises_type_error(self):
        ...
        assert_that(
            str(error.value),
            equal_to("cannot add queries for different scalar types: Boolean and Int"),
        )
    ...
    def test_adding_enum_query_to_other_query_raises_type_error(self):
        ...
        pytest.raises(
            TypeError,
            lambda: DirectionGraphType() + schema.ObjectType("Obj", fields=()),
        )
    ...
    def test_adding_enum_queries_of_different_type_raises_type_error(self):
        ...
        assert_that(
            str(error.value),
            equal_to("cannot add queries for different enum types: Direction and Size"),
        )

    def test_merged_object_query_has_fields_from_operand_queries(self):
        Song = schema.ObjectType(
            "Song",
            fields=(
        ...
        query = Song(schema.key("title", Song.fields.title())) + Song(
    ...
    def test_fields_are_recursively_merged(self):
        User = schema.ObjectType(
            "User",
            fields=lambda: (schema.field("address", type=Address),),
        )
    ...
    def test_fields_with_same_key_for_different_types_are_kept_separate(self):
        ...
        Song = schema.ObjectType(
            "Song",
            interfaces=(Item,),
            fields=(schema.field("title", type=schema.String),),
        )
        Book = schema.ObjectType(
            "Book",
            interfaces=(Item,),
            fields=(schema.field("title", type=schema.String),),
        )
        query = Item(schema.key("title", Song.fields.title())) + Item(
    ...
    def test_adding_object_query_to_other_query_raises_type_error(self):
        pytest.raises(
            TypeError, lambda: schema.ObjectType("Obj", fields=()) + schema.Boolean()
        )
    ...
    def test_adding__query_to_non_list_query_raises_type_error(self):
        pytest.raises(
            TypeError, lambda: schema.ListType(schema.Boolean)() + schema.Boolean()
        )

    def test_adding_list_query_to_list_query_of_different_element_type_raises_type_error(
        self,
    ):
        error = pytest.raises(
            TypeError,
            lambda: schema.ListType(schema.Boolean)() + schema.ListType(schema.Int)(),
        )
        assert_that(
            str(error.value),
            equal_to(
                "cannot add queries for lists with different element types: Boolean and Int"
            ),
        )
    ...
    def test_adding_nullable_query_to_non_null_query_raises_type_error(self):
        pytest.raises(
            TypeError, lambda: schema.NullableType(schema.Boolean)() + schema.Boolean()
        )

    def test_adding_nullable_query_to_nullable_query_of_different_element_type_raises_type_error(
        self,
    ):
        error = pytest.raises(
            TypeError,
            lambda: schema.NullableType(schema.Boolean)()
            + schema.NullableType(schema.Int)(),
        )
        assert_that(
            str(error.value),
            equal_to(
                "cannot add queries for nullables with different element types: Boolean and Int"
            ),
        )
...
class TestForType(object):
    ...
    def test_cannot_change_type_of_scalar_query(self):
        error = pytest.raises(
            TypeError, lambda: schema.Boolean().for_type(schema.String)
        )
        assert_that(
            str(error.value),
            equal_to("cannot coerce query for Boolean to query for String"),
        )
    ...
    def test_cannot_change_type_of_enum_query(self):
        ...
        error = pytest.raises(
            TypeError, lambda: SeasonGraphType().for_type(QuarterGraphType)
        )
        assert_that(
            str(error.value),
            equal_to("cannot coerce query for Season to query for Quarter"),
        )

    def test_converting_query_for_interface_to_object_filters_fields_to_those_for_object(
        self,
    ):
        Item = schema.InterfaceType(
            "Item", fields=(schema.field("title", type=schema.String),)
        )
        Song = schema.ObjectType(
            "Song",
            interfaces=(Item,),
    ...
    def test_cannot_convert_query_for_interface_to_unrelated_object(self):
        ...
        Book = schema.ObjectType(
            "Book", fields=(schema.field("title", type=schema.String),)
        )
        query = Item(
            schema.key("title", Item.fields.title()),
        )
        ...
        assert_that(
            str(error.value), equal_to("cannot coerce query for Item to query for Book")
        )

    def test_converting_query_for_interface_to_same_interface_keeps_fields_for_subtypes(
        self,
    ):
        Item = schema.InterfaceType(
            "Item", fields=(schema.field("title", type=schema.String),)
    ...
    def test_cannot_convert_query_for_interface_to_unrelated_interface(self):
        ...
        Song = schema.InterfaceType(
            "Song", fields=(schema.field("title", type=schema.String),)
        )
        query = Item(
            schema.key("title", Item.fields.title()),
        )
        ...
        assert_that(
            str(error.value), equal_to("cannot coerce query for Item to query for Song")
        )
    ...
    def test_cannot_convert_query_for_object_to_unrelated_object(self):
        ...
        assert_that(
            str(error.value), equal_to("cannot coerce query for Song to query for Item")
        )
    ...
    def test_list_type_for_type_calls_for_type_on_element_query(self):
        ...
        Song = schema.ObjectType(
            "Song",
            interfaces=(Item,),
            fields=(schema.field("length", type=schema.Int),),
        )
        Book = schema.ObjectType(
            "Book",
            interfaces=(Item,),
    ...
    def test_cannot_change_type_of_list_query_to_non_list_query(self):
        ...
        error = pytest.raises(
            TypeError, lambda: query.for_type(schema.NullableType(schema.Boolean))
        )
        assert_that(
            str(error.value),
            equal_to(
                "cannot coerce query for List(Boolean) to query for Nullable(Boolean)"
            ),
        )
    ...
    def test_cannot_change_type_of_nullable_query_to_non_nullable_query(self):
        ...
        assert_that(
            str(error.value),
            equal_to("cannot coerce query for Nullable(Boolean) to query for Boolean"),
        )
...
class TestFieldQuery(object):
    def test_creating_field_query_specialises_type_of_type_query(self):
        Root = schema.ObjectType(
            "Root", fields=lambda: (schema.field("song", type=Song),)
        )
        Item = schema.InterfaceType(
            "Item", fields=(schema.field("title", type=schema.String),)
        )
        ...
        field_query = Root.fields.song.query(
            key="song",
            args=(),
            type_query=Item(
                schema.key("title", Item.fields.title()),
...
class TestQueryString(object):
    ...
    def test_field_can_be_from_subtype(self):
        ...
        Book = schema.ObjectType(
            "Book", fields=(schema.field("title", schema.String),), interfaces=(Item,)
        )
        ...
        Book = schema.ObjectType(
            ...
            fields=(
                schema.field(
                    "title",
                    schema.String,
                    params=(schema.param("truncate", schema.Int),),
                ),
            ),
        )

        query = schema.key(
            "title", Book.fields.title(Book.fields.title.params.truncate(42))
        )
...
class TestCollectTypes(object):
    def test_boolean_has_no_child_types(self):
        assert_that(
            schema.collect_types((schema.Boolean,)), contains_exactly(schema.Boolean)
        )

    def test_float_has_no_child_types(self):
        assert_that(
            schema.collect_types((schema.Float,)), contains_exactly(schema.Float)
        )
    ...
    def test_string_has_no_child_types(self):
        assert_that(
            schema.collect_types((schema.String,)), contains_exactly(schema.String)
        )
    ...
    def test_enum_has_no_child_types(self):
        ...
        assert_that(
            schema.collect_types((DirectionGraphType,)),
            contains_exactly(DirectionGraphType),
        )

    def test_interface_type_has_field_types_in_child_types(self):
        User = schema.InterfaceType(
            "User", fields=lambda: (schema.field("name", type=schema.String),)
        )
    ...
    def test_input_object_type_has_field_types_in_child_types(self):
        UserInput = schema.InputObjectType(
            "UserInput", fields=lambda: (schema.field("name", type=schema.String),)
        )
        ...
        User = schema.InterfaceType(
            ...
            fields=lambda: (
                schema.field(
                    "name",
                    type=schema.String,
                    params=(schema.param("long", type=schema.Boolean),),
                ),
            ),
        )
        collected_types = schema.collect_types((User,))
    ...
    def test_list_type_has_element_type_as_child_type(self):
        ...
        assert_that(
            collected_types,
            contains_exactly(schema.ListType(schema.String), schema.String),
        )
    ...
    def test_nullable_type_has_element_type_as_child_type(self):
        ...
        assert_that(
            collected_types,
            contains_exactly(schema.NullableType(schema.String), schema.String),
        )
    ...
    def test_object_type_has_interface_types_in_child_types(self):
        Person = schema.InterfaceType(
            "Person", fields=lambda: (schema.field("name", type=schema.String),)
        )
        User = schema.ObjectType(
            "User",
            fields=lambda: (schema.field("name", type=schema.String),),
            interfaces=(Person,),
        )
        collected_types = schema.collect_types((User,))
...
@pytest.mark.parametrize(
    "graph_type, element_type, expected",
    (
        (
            schema.ListType(schema.Boolean),
            schema.String,
            schema.ListType(schema.String),
        ),
        (
            schema.NullableType(schema.Boolean),
            schema.String,
            schema.NullableType(schema.String),
        ),
...
def test_replace_element_type(graph_type, element_type, expected):
    assert_that(
        schema.replace_element_type(graph_type, element_type), equal_to(expected)
    )
