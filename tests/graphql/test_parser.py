...
def test_simple_mutation_is_converted_to_object_query():
    ...
    object_query = _document_text_to_graph_query(
        graphql_query, query_type=QueryRoot, mutation_type=MutationRoot
    )
...
def test_given_no_mutation_type_is_defined_when_operation_is_mutation_then_error_is_raised():
    ...
    assert_that(
        error.value.nodes, is_sequence(has_attrs(operation=has_attrs(value="mutation")))
    )
...
def test_fields_can_be_nested():
    Root = g.ObjectType(
        "Root",
        fields=lambda: (g.field("one", type=One),),
    )

    One = g.ObjectType(
        "One",
        fields=lambda: (g.field("two", type=Two),),
    )

    Two = g.ObjectType(
        "Two",
        fields=lambda: (g.field("three", type=g.Int),),
    )
...
def test_can_request_fields_of_list():
    Root = g.ObjectType(
        "Root",
        fields=lambda: (g.field("one", type=g.ListType(One)),),
    )

    One = g.ObjectType(
        "One",
        fields=lambda: (g.field("two", type=g.Int),),
    )
...
def test_fragments_can_be_on_more_specific_type():
    ...
    object_query = _document_text_to_graph_query(
        graphql_query, query_type=Root, types=(Cat,)
    )
...
def test_fragments_are_recursively_merged():
    Root = g.ObjectType(
        "Root",
        fields=lambda: (g.field("value", type=g.Int),),
    )
...
def test_fragment_can_be_spread_into_list_type():
    User = g.ObjectType("User", fields=lambda: (g.field("name", type=g.String),))

    Root = g.ObjectType(
        "Root",
        fields=lambda: (g.field("user", type=g.ListType(User)),),
    )
...
def test_fragment_can_be_spread_into_nullable_type():
    ...
    Root = g.ObjectType(
        "Root",
        fields=lambda: (g.field("user", type=g.NullableType(User)),),
    )
...
def test_fragment_fields_are_updated_to_fields_for_element_type():
    ...
    Root = g.ObjectType(
        "Root",
        fields=lambda: (g.field("users", type=g.ListType(g.NullableType(User))),),
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            g.InputObjectType(
                "Casing", fields=(g.input_field("field_zero", type=g.Int),)
            ),
            "{fieldZero: 1}",
...
# TODO: deduplicate arg/variable tests
@pytest.mark.parametrize(
    "graph_type, graphql_type, variable_value, arg_value",
    [
...
def test_graphql_arg_values_from_variables_are_converted(
    ...
    object_query = _document_text_to_graph_query(
        graphql_query, query_type=Root, variables={"var": variable_value}
    )
...
def test_when_null_variable_is_missing_then_variable_is_null():
    ...
    object_query = _document_text_to_graph_query(
        graphql_query, query_type=Root, variables={}
    )
...
def test_when_non_null_variable_is_missing_then_error_is_raised():
    ...
    error = pytest.raises(
        GraphQLError,
        lambda: _document_text_to_graph_query(
            graphql_query, query_type=Root, variables={}
        ),
    )
    assert_that(
        error.value.message,
        equal_to("Variable '$var' of required type 'Int!' was not provided."),
    )
...
def test_when_input_object_variable_is_missing_field_then_error_is_raised():
    Input = g.InputObjectType(
        "Input",
        fields=lambda: (g.input_field("field", type=g.Int),),
    )
    ...
    error = pytest.raises(
        GraphQLError,
        lambda: _document_text_to_graph_query(
            graphql_query, query_type=Root, variables=variables
        ),
    )
    assert_that(
        error.value.message,
...
def test_when_arg_is_not_set_then_default_is_used():
    ...
    object_query = _document_text_to_graph_query(graphql_query, query_type=Root)
    assert_that(
        object_query.field_queries[0].args,
        has_attrs(
            arg0=None,
            arg1=42,
        ),
...
def test_when_field_value_is_not_set_then_default_is_used():
    ...
    object_query = _document_text_to_graph_query(graphql_query, query_type=Root)
    assert_that(
        object_query.field_queries[0].args.arg,
        has_attrs(
            field0=None,
            field1=42,
        ),
...
def test_query_is_validated():
    ...
    error = pytest.raises(
        GraphQLError,
        lambda: _document_text_to_graph_query(graphql_query, query_type=Root),
    )
...
def _document_text_to_graph_query(
    document_text, *, query_type, mutation_type=None, types=None, variables=None
):
    schema = create_graphql_schema(
        query_type=query_type, mutation_type=mutation_type, types=types
    )
    return document_text_to_query(
        document_text, graphql_schema=schema, variables=variables
    ).graph_query
